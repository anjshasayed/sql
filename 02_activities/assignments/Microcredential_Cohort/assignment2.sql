...
--QUERY 3
-- Reverse numbering: most recent visit = 1
WITH visits AS (
  SELECT
    customer_id,
    market_date,
...
SELECT market_date, total_sales, 'best_day' AS label
FROM ranked
WHERE best_rank = 1

UNION

SELECT market_date, total_sales, 'worst_day' AS label
FROM ranked
WHERE worst_rank = 1;
...
WHERE product_id IN (SELECT product_id FROM latest_quantity);
